...
export function MembershipModal({ isOpen, onClose }: MembershipModalProps) {
  ...
  const { data: fetchedThresholds } = useQuery<{ GOLD: number; VIP: number }>({
    queryKey: ['https://laundry-be-admin.onrender.com/api/membership-thresholds'],
    enabled: isOpen,
  });
  ...
  const updateMembershipMutation = useMutation({
    mutationFn: async ({ customerId, membershipLevel }: { customerId: number; membershipLevel: string }) => {
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/customers/${customerId}`, {
        method: 'PUT',
        headers: {
  ...
  const updateThresholdsMutation = useMutation({
    mutationFn: async (newThresholds: { GOLD: number; VIP: number }) => {
      const response = await fetch('https://laundry-be-admin.onrender.com/api/membership-thresholds', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['https://laundry-be-admin.onrender.com/api/membership-thresholds'] });
      queryClient.invalidateQueries({ queryKey: ['https://laundry-be-admin.onrender.com/api/customers'] });
      toast({
        title: "common.success",
