...
export function EmployeeFormModal({
  ...
  const generateEmployeeId = async () => {
    try {
      const response = await apiRequest("GET", "https://laundry-be-admin.onrender.com/api/employees/next-id");
  ...
  const createMutation = useMutation({
    mutationFn: async (data: InsertEmployee) => {
      const response = await apiRequest("POST", "https://laundry-be-admin.onrender.com/api/employees", data);
      if (!response.ok) {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/employees"] });
      toast({
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: InsertEmployee) => {
      const response = await apiRequest(
        "PUT",
        `https://laundry-be-admin.onrender.com/api/employees/${employee?.id}`,
        data,
      );
      if (!response.ok) {
        const errorData = await response.json();
