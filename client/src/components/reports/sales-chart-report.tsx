...
export function SalesChartReport({ isAdmin }: { isAdmin?: boolean }) {
  ...
  const { data: storeSettings, isLoading: storesLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch("https://laundry-be-admin.onrender.com/api/store-settings");
      if (!response.ok) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        const url = `https://laundry-be-admin.onrender.com/api/orders/date-range/${startDateTimeISO}/${endDateTimeISO}${floorFilter}?${queryString}`;
  ...
  const { data: storesFilterData = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://laundry-be-admin.onrender.com/api/store-settings/list");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  const { data: orderItems = [], isLoading: orderItemsLoading } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/order-items"],
    queryFn: async () => {
      try {
        const response = await fetch("https://laundry-be-admin.onrender.com/api/order-items");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const { data: products } = useQuery({
    queryKey: [
      "https://laundry-be-admin.onrender.com/api/products",
      selectedCategory,
      productType,
      productSearch,
      startDate,
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/products${storeParam}`);
  ...
  const { data: categories } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/categories", storeFilter],
    queryFn: async () => {
      const storeParam =
        storeFilter && storeFilter !== "all" ? `?storeCode=${storeFilter}` : "";
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/categories${storeParam}`);
  ...
  const { data: customers } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/customers", customerSearch, customerStatus],
    queryFn: async () => {
      const response = await fetch(
        `https://laundry-be-admin.onrender.com/api/customers/${customerSearch || "all"}/${customerStatus}`,
      );
  ...
  const { data: generalSettings } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/general-settings/by-code"],
    queryFn: async () => {
      const response = await fetch("https://laundry-be-admin.onrender.com/api/general-settings/by-code/ST-002");
  ...
  const {
    ...
    queryKey: [
      "https://laundry-be-admin.onrender.com/api/product-analysis",
      startDate,
      endDate,
      startTime,
      endTime,
    ...
    queryFn: async () => {
      try {
        ...
        const response = await fetch(
          `https://laundry-be-admin.onrender.com/api/product-analysis/${encodeURIComponent(startDateTimeLocal)}/${encodeURIComponent(endDateTimeLocal)}${floorFilter}${storeCodeFilter}?${params}`,
          {
            method: "GET",
            headers: {
  ...
  const { data: transactions } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/transactions"],
    staleTime: 5 * 60 * 1000,
  });
