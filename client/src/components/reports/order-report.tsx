...
export function OrderReport() {
  ...
  const { data: orders = [] } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/orders/date-range", startDate, endDate, "all"],
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://laundry-be-admin.onrender.com/api/orders/date-range/${startDate}/${endDate}/all`,
        );
        if (!response.ok) {
  ...
  const { data: transactions = [] } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/transactions", startDate, endDate],
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://laundry-be-admin.onrender.com/api/transactions/${startDate}/${endDate}`,
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
