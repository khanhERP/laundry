...
export function DashboardOverview() {
  ...
  const {
    ...
    queryKey: ["https://laundry-be-admin.onrender.com/api/orders/list", startDate, endDate, "all", storeFilter],
    queryFn: async () => {
      try {
        ...
        const response = await fetch(`https://laundry-be-admin.onrender.com/api/orders/list?${params.toString()}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const handleRefresh = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/orders/date-range"] });
