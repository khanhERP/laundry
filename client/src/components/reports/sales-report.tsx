...
export function SalesReport() {
  ...
  const {
    ...
    queryKey: ["https://laundry-be-admin.onrender.com/api/order-items/date-range", startDate, endDate, "all"],
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://laundry-be-admin.onrender.com/api/order-items/${startDate}/${endDate}`,
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
