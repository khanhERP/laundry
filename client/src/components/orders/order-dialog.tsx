...
export function OrderDialog({
  ...
  const { data: existingOrderItems, refetch: refetchExistingItems } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/order-items", existingOrder?.id],
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest(
        "GET",
        `https://laundry-be-admin.onrender.com/api/order-items/${existingOrder.id}`,
      );
        ...
        if (mode === "edit" && existingOrder) {
          ...
          if (orderData.items.length > 0) {
            ...
            const addItemsResponse = await apiRequest(
              "POST",
              `https://laundry-be-admin.onrender.com/api/orders/${existingOrder.id}/items`,
              {
          ...
          if (shouldRecalculate) {
            ...
            try {
              const recalcResponse = await apiRequest(
                "POST",
                `https://laundry-be-admin.onrender.com/api/orders/${existingOrder.id}/recalculate`,
              );
          ...
          const updateResponse = await apiRequest(
            "PUT",
            `https://laundry-be-admin.onrender.com/api/orders/${existingOrder.id}`,
            {
          ...
          const response = await apiRequest("POST", "https://laundry-be-admin.onrender.com/api/orders", orderData);
