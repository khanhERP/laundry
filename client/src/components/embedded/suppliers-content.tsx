...
export default function SuppliersPageContent() {
  ...
  const handleDeleteSupplier = async (supplierId: number) => {
    ...
    try {
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/suppliers/${supplierId}`, { method: "DELETE" });
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);

      await queryClient.refetchQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/suppliers"] });
