...
export default function EmployeesPageContent() {
  ...
  const handleDeleteEmployee = async (employeeId: number) => {
    ...
    try {
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/employees/${employeeId}`, { method: "DELETE" });
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);

      await queryClient.refetchQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/employees"] });
