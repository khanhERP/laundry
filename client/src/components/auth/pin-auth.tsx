...
export function PinAuth({ onAuthSuccess }: PinAuthProps) {
  ...
  const verifyToken = async (token: string) => {
    try {
      const response = await fetch("https://laundry-be-admin.onrender.com/api/auth/verify", {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Call login API
      const response = await fetch("https://laundry-be-admin.onrender.com/api/auth/login", {
        method: "POST",
        headers: {
