...
export function ReceiptModal({
  ...
  const { data: tableInfo } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/tables/by-order", receipt?.id],
    queryFn: async () => {
      ...
      const orderResponse = await apiRequest(
        "GET",
        `https://laundry-be-admin.onrender.com/api/orders/${receipt.id}`,
      );
      ...
      const tableResponse = await apiRequest(
        "GET",
        `https://laundry-be-admin.onrender.com/api/tables/${order.tableId}`,
      );
  ...
  useEffect(() => {
    async function fetchPrinterConfigs() {
      try {
        const printerResponse = await fetch("https://laundry-be-admin.onrender.com/api/printer-configs");
        ...
        if (receipt?.tableId) {
          try {
            const tableResponse = await fetch(`https://laundry-be-admin.onrender.com/api/tables/${receipt.tableId}`);
            if (tableResponse.ok) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (activePrinterConfigs.length > 0) {
        ...
        try {
          const printResponse = await fetch("https://laundry-be-admin.onrender.com/api/pos/print-receipt", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
