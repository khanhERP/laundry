...
export function POSHeader({ onLogout }: POSHeaderProps) {
  ...
  const { data: todayAttendance } = useQuery<AttendanceRecord[]>({
    queryKey: ["https://laundry-be-admin.onrender.com/api/attendance", todayDate],
    queryFn: async () => {
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/attendance?date=${todayDate}`);
      if (!response.ok) {
  ...
  const handleLogout = async () => {
    try {
      ...
      await fetch("https://laundry-be-admin.onrender.com/api/auth/logout", {
        method: "POST",
        credentials: "include",
      });
    } catch (error) {
