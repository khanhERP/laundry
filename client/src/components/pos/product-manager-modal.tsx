...
export function ProductManagerModal({
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: (newProduct) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/products/active"] });
  ...
  const updateProductMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/products/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(finalData),
      });
