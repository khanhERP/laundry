...
export default function ExpenseVoucherModal({
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://laundry-be-admin.onrender.com/api/customers");
  ...
  const { data: employees = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://laundry-be-admin.onrender.com/api/employees");
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        return Array.isArray(data) ? data : [];
      } catch (error) {
  ...
  const createVoucherMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/expense-vouchers"] });
  ...
  const updateVoucherMutation = useMutation({
    mutationFn: async (data: ExpenseVoucher) => {
      ...
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/expense-vouchers/${data.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  const deleteVoucherMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/expense-vouchers/${id}`, {
        method: "DELETE",
      });
