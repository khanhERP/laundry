...
export function InvoiceManagementModal({
  ...
  const { data: invoices = [], isLoading } = useQuery<Invoice[]>({
    ...
    queryFn: async () => {
      const response = await apiRequest("GET", "https://laundry-be-admin.onrender.com/api/invoices");
  ...
  const { data: invoiceItems = [] } = useQuery<InvoiceItem[]>({
    queryKey: ["https://laundry-be-admin.onrender.com/api/invoice-items", selectedInvoice?.id],
    queryFn: async () => {
      if (!selectedInvoice?.id) return [];
      const response = await apiRequest("GET", `https://laundry-be-admin.onrender.com/api/invoice-items/${selectedInvoice.id}`);
      return response.json();
    },
