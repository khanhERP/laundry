...
export function ShoppingCart({
  ...
  const fetchCustomers = async (searchTerm: string) => {
    try {
      setIsSearching(true);
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/customers?search=${searchTerm}`);
      if (!response.ok) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Refresh orders list
      await queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/orders"] });
    } catch (error) {
