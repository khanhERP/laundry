...
export function PointsManagementModal({ isOpen, onClose }: PointsManagementModalProps) {
  ...
  const { data: pointTransactions, isLoading: transactionsLoading } = useQuery<PointTransaction[]>({
    queryKey: ['https://laundry-be-admin.onrender.com/api/point-transactions'],
  ...
  const adjustPointsMutation = useMutation({
    mutationFn: async ({ customerId, points, type, description }: { 
      ...
      const response = await apiRequest('POST', 'https://laundry-be-admin.onrender.com/api/customers/adjust-points', {
        customerId,
        points,
        type,
        description
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['https://laundry-be-admin.onrender.com/api/point-transactions'] });
  ...
  const processPaymentMutation = useMutation({
    mutationFn: async ({ customerId, points }: { customerId: number; points: number }) => {
      const response = await apiRequest('POST', 'https://laundry-be-admin.onrender.com/api/customers/redeem-points', {
        customerId,
        points
      });
      return response.json();
