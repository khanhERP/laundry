...
export function GeneralSettingsContent() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, isActive, settingCode }: { id: number; isActive: boolean; settingCode?: string }) => {
      ...
      if (settingCode === "ST-002") {
        ...
        if (st003) {
          ...
          await fetch(`https://laundry-be-admin.onrender.com/api/general-settings/${st003.id}`, {
            method: "PUT",
        ...
        if (st002) {
          ...
          await fetch(`https://laundry-be-admin.onrender.com/api/general-settings/${st002.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ isActive: !isActive }),
            credentials: "include",
      ...
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/general-settings/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/general-settings"] });
      toast({
        title: "Thành công",
