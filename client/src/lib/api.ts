export const loginPosAsync = async (request: LoginPosRequest): Promise<LoginResponse> => {
  const response = await fetch('https://laundry-be-admin.onrender.com/api/pos/login', {
    method: 'POST',
    headers: {
...
export const createQRPosAsync = async (request: CreateQRPosRequest, bankCode: string, clientID: string): Promise<CreateQRPosResponse> => {
  try {
    ...
    const response = await fetch(`https://laundry-be-admin.onrender.com/api/pos/create-qr-proxy`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    try {
      const fallbackResponse = await fetch(`https://laundry-be-admin.onrender.com/api/pos/create-qr?bankCode=${bankCode}&clientID=${clientID}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
