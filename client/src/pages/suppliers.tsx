...
export default function SuppliersPage({ onLogout }: SuppliersPageProps) {
  ...
  const { data: suppliers, isLoading } = useQuery({
    queryKey: ['https://laundry-be-admin.onrender.com/api/suppliers', { status: selectedStatus, search: searchQuery }],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (selectedStatus !== 'all') params.append('status', selectedStatus);
      if (searchQuery) params.append('search', searchQuery);
      
      const response = await apiRequest('GET', `https://laundry-be-admin.onrender.com/api/suppliers?${params}`);
  ...
  const { data: supplierStats } = useQuery({
    queryKey: ['https://laundry-be-admin.onrender.com/api/purchase-orders/supplier-stats'],
    queryFn: async () => {
      const response = await apiRequest('GET', 'https://laundry-be-admin.onrender.com/api/purchase-orders/supplier-stats');
      return response.json();
    },
  });
  ...
  const deleteSupplierMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest('DELETE', `https://laundry-be-admin.onrender.com/api/suppliers/${id}`);
      return response.json();
    },
    onSuccess: () => {
