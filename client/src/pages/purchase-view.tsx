...
export default function PurchaseViewPage({ onLogout, onSuccess, hideBackButton = false, purchaseId: propPurchaseId }: PurchaseViewPageProps) {
  ...
  const { data: paymentMethodsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch("https://laundry-be-admin.onrender.com/api/payment-methods");
  ...
  const { data: purchaseOrder, isLoading: isOrderLoading, error: orderError } = useQuery<PurchaseOrder>({
    queryKey: ["https://laundry-be-admin.onrender.com/api/purchase-receipts", purchaseId],
    queryFn: async () => {
      if (!purchaseId) throw new Error("Purchase ID not found");

      console.log('🔍 Fetching purchase receipt with ID:', purchaseId);
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-receipts/${purchaseId}`);
  ...
  const { data: purchaseItems = [], isLoading: isItemsLoading } = useQuery<PurchaseReceiptItem[]>({
    queryKey: ["https://laundry-be-admin.onrender.com/api/purchase-receipts", purchaseId, "items"],
    queryFn: async () => {
      if (!purchaseId) throw new Error("Purchase ID not found");

      const response = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-receipts/${purchaseId}/items`);
      if (!response.ok) {
  ...
  const { data: attachedDocuments = [] } = useQuery({
    queryKey: ["https://laundry-be-admin.onrender.com/api/purchase-receipts", purchaseId, "documents"],
    queryFn: async () => {
      if (!purchaseId) throw new Error("Purchase ID not found");

      const response = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-receipts/${purchaseId}/documents`);
      if (!response.ok) {
        return [];
      }
  ...
  const handleAddNewRow = () => {
    ...
    queryClient.setQueryData(
      ["https://laundry-be-admin.onrender.com/api/purchase-receipts", purchaseId, "items"],
      [...syncedItems, newItem]
    );
  ...
  const handleFileDownload = async (document: any) => {
    try {
      ...
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-receipts/documents/${document.id}/download`, {
        method: 'GET',
        headers: {
  ...
  const handleFileDelete = async (documentId: number) => {
    try {
      ...
      const response = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-receipts/documents/${documentId}`, {
        method: 'DELETE',
      });
  ...
  const handleSave = async () => {
    try {
      ...
      const receiptResponse = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-receipts/${purchaseId}`, {
        method: 'PUT',
      ...
      if (existingItemIds.length > 0) {
        for (const itemId of existingItemIds) {
          try {
            ...
            const response = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-order-items/${itemId}`, {
              method: 'DELETE'
            });
      ...
      for (let index = 0; index < purchaseItems.length; index++) {
        ...
        try {
          ...
          const response = await fetch('https://laundry-be-admin.onrender.com/api/purchase-order-items', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
                                  ...
                                  <Button
                                    ...
                                    onClick={async () => {
                                      ...
                                      if (item.id && item.id < 1000000000) { // Assuming valid IDs are smaller than temporary IDs
                                        try {
                                          const response = await fetch(`https://laundry-be-admin.onrender.com/api/purchase-order-items/${item.id}`, {
                                            method: 'DELETE',
                                          });
