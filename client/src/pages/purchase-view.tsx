...
export default function PurchaseViewPage({ onLogout, onSuccess, hideBackButton = false, purchaseId: propPurchaseId }: PurchaseViewPageProps) {
  ...
  const { data: attachedDocuments = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/purchase-receipts/${purchaseId}/documents`);
      if (!response.ok) {
        return [];
      }
  ...
  const handleFileDownload = async (document: any) => {
    try {
      ...
      const response = await fetch(`/api/purchase-receipts/documents/${document.id}/download`, {
        method: 'GET',
        headers: {
  ...
  const handleFileDelete = async (documentId: number) => {
    try {
      ...
      const response = await fetch(`/api/purchase-receipts/documents/${documentId}`, {
        method: 'DELETE',
      });
  ...
  const handleSave = async () => {
    try {
      ...
      const receiptResponse = await fetch(`/api/purchase-receipts/${purchaseId}`, {
        method: 'PUT',
      ...
      if (existingItemIds.length > 0) {
        for (const itemId of existingItemIds) {
          try {
            ...
            const response = await fetch(`/api/purchase-order-items/${itemId}`, {
              method: 'DELETE'
            });
                                  ...
                                  <Button
                                    ...
                                    onClick={async () => {
                                      ...
                                      if (item.id && item.id < 1000000000) { // Assuming valid IDs are smaller than temporary IDs
                                        try {
                                          const response = await fetch(`/api/purchase-order-items/${item.id}`, {
                                            method: 'DELETE',
                                          });
