...
export default function PurchaseFormPage({
  ...
  const { data: existingOrder, isLoading: isLoadingOrder } = useQuery({
    queryKey: [`https://laundry-be-admin.onrender.com/api/purchase-orders/${id}`],
    enabled: Boolean(id),
    select: (data: any) => {
  ...
  const { data: existingDocuments } = useQuery({
    queryKey: [`https://laundry-be-admin.onrender.com/api/purchase-orders/${id}/documents`],
    enabled: Boolean(id),
    select: (data: any) => data || [],
  });
  ...
  const {
    ...
    queryKey: ["https://laundry-be-admin.onrender.com/api/purchase-orders/next-po-number"],
    enabled: !isEditMode,
    queryFn: async () => {
      try {
        ...
        const response = await apiRequest(
          "GET",
          "https://laundry-be-admin.onrender.com/api/purchase-orders/next-po-number",
        );
        if (!response.ok) {
  ...
  const createProductMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await apiRequest("POST", "https://laundry-be-admin.onrender.com/api/products", data);
    ...
    onSuccess: (newProduct) => {
      ...
      queryClient.setQueryData(["https://laundry-be-admin.onrender.com/api/products"], (old: any[]) => {
        return [
          ...(old || []),
  ...
  const onSubmit = async (
    ...
    try {
      ...
      const response = isEditMode
        ? await fetch(`https://laundry-be-admin.onrender.com/api/purchase-receipts/${id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(submissionData),
          })
        : await fetch("https://laundry-be-admin.onrender.com/api/purchase-receipts", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(submissionData),
          });
        ...
        const uploadPromises = attachedFiles.map(async (fileData) => {
          ...
          try {
            ...
            const uploadResponse = await fetch(
              `https://laundry-be-admin.onrender.com/api/purchase-receipts/${result.id}/documents`,
              {
                method: "POST",
                headers: {
      ...
      queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/purchase-receipts"] });
      queryClient.invalidateQueries({ queryKey: ["https://laundry-be-admin.onrender.com/api/suppliers"] });
